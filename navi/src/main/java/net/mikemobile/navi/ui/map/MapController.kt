...
interface OnMapControllerListener {
    ...
    fun onChangeCurrentTest(point: Point)
...
interface MapInterface {
    ...
    fun onCurrent(latLng: LatLng)       // 現在位置を表示する
...
class MapController(val context: Context, val mMap: GoogleMap, val listener: OnMapControllerListener): MapInterface {
    companion object {
        ...
        enum class MAPLOAD_AND_ZOOM_STEP{
            ...
            MAP_LOAD, //地図読み込み中
            MAP_LOADED, //地図読み込み中
            MAP_FIRST_ZOOM, //地図読み込み中
            MAP_ZOOM, //初回位置移動中
            MAP_ZOOM_END, //初回位置移動中
            START_MAP //マップ表示開始
        }
    ...
    private fun setFocus(latLng: LatLng, isAnimation: Boolean = true) {
        var _latLng = calcMoveCenterToLatLng(latLng)
    ...
    private fun setTwoPointCenter(_south: LatLng, _north: LatLng) {
        ...
        try {
            ...
            val bounds = CameraUpdateFactory.newLatLngBounds(australiaBounds, width, height, width/ 20)
    ...
    override fun onCurrent(latLng: LatLng) {
